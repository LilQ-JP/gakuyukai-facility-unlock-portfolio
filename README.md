...
公開用に確認した画面です。実在の予約内容、暗証番号、認証情報は表示していません。SwitchBot モニター画面のデバイス識別子と運用者名は匿名化しています。

### 団体向け入口

団体登録、ログイン、予約申請へ進むためのトップ画面です。

![団体向けトップ画面](docs/images/organization-portal.png)

### 管理ダッシュボード

承認待ち申請、登録団体、施設の空き状況を一画面で確認します。

![管理ダッシュボード](docs/images/admin-dashboard.png)

### SwitchBot デバイスモニター

Keypad と Lock を分けて一覧化し、バッテリー残量・取得可否を確認できます。

![匿名化した SwitchBot デバイスモニター](docs/images/switchbot-monitor-redacted.png)

## 解決した課題

学生団体が管理するコピー室・会議室にはスマートロックが設置されていましたが、担当者が不在の日に利用希望が出ると、担当者が現地でカードキーをかざして解錠する必要がありました。この運用では、利用者も担当者も場所と時間に縛られます。

そこで、利用団体が「誰が、いつ、どの施設を、どのような目的で使いたいか」を申請し、会長・副会長などの責任者が内容を遠隔で確認して承認または非承認にできる流れを作りました。承認された予約には、利用時間だけ有効な SwitchBot の一時暗証番号を発行します。利用時間を過ぎると、その番号は使えません。

さらに、複数施設に設置した SwitchBot を横断して確認できるようにし、標準アプリでは把握しづらかったデバイスごとのバッテリー残量をパーセント表示で確認できるようにしました。これにより、電池切れによる利用不能を事前に把握しやすくしています。
